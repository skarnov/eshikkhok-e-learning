<?php
global $multilingualFields, $JACK_SETTINGS;
...
if(!$this->default_role){
    add_notification('Default role for instructors is not set. Please set the default role for instructors.', 'error');
...
if ($edit) {
    ...
    if (!$user) {
        add_notification('Instructor not found for editing.', 'error');
        header('location:' . $myUrl);
        exit();
        }
    else $user = $user['data'];
...
if($_POST){
    ...
    $data['user_meta_type'] = 'instructor';
    ...
    if($ret['error']){
        ...
        add_notification('The instructor has been '.($edit ? 'updated.':'added.'),'success');
        user_activity::add_activity('The instructor (ID: '.$user_id.') has been '.($edit ? 'updated.':'created.'),'success', ($edit ? 'update':'create'));
...
<div class="page-header">
    <h1><?php echo $edit ? 'Update Instructor' : 'New Instructor' ?></h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            ...
            <?php if (has_permission('add_instructor')): ?>
                <?php
                echo linkButtonGenerator(array(
                    'href' => build_url(array('action' => 'add_edit_instructor'), array('edit')),
                    'action' => 'add',
                    'icon' => 'icon_add',
                    'text' => 'New Instructor',
...
/*                            }
        ...
        <div class="panel-footer tar">
            ...
            echo submitButtonGenerator(array(
                ...
                'text' => $edit ? 'Update Instructor' : 'Save Instructor',
                'title' => $edit ? 'Update Instructor' : 'Save Instructor',
                'size' => ''
            ));
            ?>
        </div>
