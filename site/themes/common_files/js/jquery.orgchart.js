(function($) {
    ...
    $.fn.orgChart.defaults = {
        data: [{id:1, designation:0,name: 'Mr.X', img:null, parent: 0}],
        source: [],
        showControls: true,
        allowEdit: true,
        onAddNode: null,
    ...
    function OrgChart($container, opts){
        ...
        this.startEdit = function(id){
            ...
            var html =  '<select class="org-select title col-sm-12">';
            for(var i in opts.source){
                html += '<option value="'+i+'" '+(i==nodes[id].data.designation ? 'selected' : '')+'>'+opts.source[i]+' </option>';
                }
            html += '</select>';
            ...
            /*var commitChange = function(){
                var h2Element = $('<h2 data-designation="'+nodes[id].data.val+'" class="text-danger m0 text-xs text-bold">'+nodes[id].data.name+'</h2>');
                if(opts.allowEdit){
            ...
            inputElement.keyup(function(event){
                if(event.which == 13){
                    ...
                    nodes[id].data.name = $(html+' option:selected').text();
                    nodes[id].data.val = $(html+' option:selected').val();
                }
            });
        ...
        this.editDesignation = function(id){
            var paraElement = $('<input class="org-input designation" type="text" value="'+nodes[id].data.name+'"/>');
            $container.find('div[node-id='+id+'] p').replaceWith(paraElement);

            var commitChangedesignation = function(){

                var pElement = $('<p class="text-danger m0 text-xs">'+nodes[id].data.name+'</p>');
                if(opts.allowEdit){
                    pElement.click(function(){
                        self.editDesignation(id);
                        })
                    }
                paraElement.replaceWith(pElement);
                }

            paraElement.focus();
            paraElement.keyup(function(event){
                if(event.which == 13){
                    commitChangedesignation();
                    }
                else{
                    nodes[id].data.designation = paraElement.val();
                    }
                });
            paraElement.blur(function(event){
                commitChangedesignation();
                })
            }
        this.newNode = function(parentId){
            ...
            self.addNode({id: nextId, designation: 0,name: 'Mr.X', img:null, parent: parentId });
            }
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.designation !== 'undefined'){
                var html =  '<select class="org-select title col-sm-12">';
                for(var i in opts.source){
                    html += '<option value="'+i+'" '+(i==self.data.designation ? 'selected' : '')+'>'+opts.source[i]+' </option>';
                }
                html += '</select>';
                nameString = html;
                }
            if(data.img != null){
                imgstr = '<span class="fl"><img src="'+self.data.img+'" alt="'+self.data.name+'" width="50px" height="50px"/></span>'
                }
            if(data.name != null){
                descString = '<p class="text-danger m0 text-xs">'+self.data.name+'</p>';
            }
            ...
            return "<div class='node' node-id='"+this.data.id+"' parent-id='"+this.data.parent+"'>"+imgstr+nameString+descString+buttonsHtml+"</div>";
        }
    }
