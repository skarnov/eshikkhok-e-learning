...
function pagination($total, $per_page, $start, $items = 5, $style = 'table_footer'){
    ...
    elseif($style == 'table_footer'){
        $_data = "<div class='DT-pagination'><ul class='pagination m0'>";
...
function build_url($replace = array(),$delete = array()){
...
// language
    ...
    <head>
        ...
        <script type="text/javascript" src="js/include.min.js?v=1"></script>
    </head>
    <body>
    ...
    $files = scan_dir($current_path.$rfm_subfolder.$subdir);
    $n_files=count($files);
    $start = isset($_GET['start']) && $_GET['start'] ? $_GET['start'] : 0;
    $perpage = 20;
    $this_start = $start*$perpage;
    $sorted=array();
    $current_folder=array();
    $prev_folder=array();
    echo $pagination =  pagination($n_files, $perpage, $start);
    $files = array_slice($files,$start*$perpage,$perpage);
    if($start){
        $temp = array('.','..');
