...
$paths = explode('/',$_SERVER['SERVER_PROTOCOL']);
$paths = strtolower($paths[0]);
$paths = $paths.'://'.$_SERVER['SERVER_NAME'].($additional ? '/'.$additional : '');
$paths = str_replace('/site/themes/common_files/filemanager','',$paths);

session_start();

if(!isset($_SESSION['admin_loggedin'])) die('Access Denied');
else if(!$_SESSION['admin_loggedin']) die('Access Denied');
...
$base_url =
    $paths.'/site/contents'; // DON'T TOUCH (base url (only domain) of site (without final /)).
$upload_dir = '/uploads/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../../contents/uploads/'; // relative path from filemanager folder to upload folder (with final /)
//thumbs folder can't put inside upload folder
$thumbs_base_path = '../../../contents/thumbs/'; // relative path from filemanager folder to thumbs folder (with final /)
...
$key = md5('3devs@4321');
$public_user_key = md5('public_user_key_3devs@4321');
$access_keys = array($key, $public_user_key);
...
$previewable_text_file_exts = array();// array('txt', 'log', 'xml','html','css','htm','js');
$previewable_text_file_exts_no_prettify = array();// array('txt', 'log');

// you can edit these type of files if $edit_text_files is true (only text based files)
// you can create these type of files if $create_text_files is true (only text based files)
// if you want you can add html,css etc. 
// but for security reasons it's NOT RECOMMENDED!
$editable_text_file_exts = array();//array('txt', 'log', 'xml','html','css','htm','js');
