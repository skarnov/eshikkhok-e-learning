...
if($_GET['key'] && $_GET['confirm_reset'] == 'yes'){
	...
	if($ret['error']){
		...
		add_notification('Password Reset Successful. <strong>Please check your email for changed password.</strong><br /><br />You will be redirected shortly ...','success');
...
<!--[if gt IE 9]><!--> <html class="gt-ie8 gt-ie9 not-ie"> <!--<![endif]-->
    <head>
        ...
        <title>Reset Your Password - <?php echo processToRender($_config['site_name']); ?></title>
    ...
    <div class="signin-header">
        ...
        <form action="" id="registration-form_id" class="panel login_form" method="post">
            ...
            <h1 class="form-header" style="color:<?php echo $_config['admin_login_prompt_text_color'] ? $_config['admin_login_prompt_text_color'] : '#555555'?>;margin-top: 0">Password Reset</h1>
            <?php echo $notify_user->get_notification(); ?>
            <div style="margin-bottom: 10px;"></div>
            <div class="note note-info">
                <h4 class="note-title">Please Note</h4>
                Your password will now be restored and a new password will be sent to your email address. Please change the the password after using it for the first time.
            </div>
            <a href="<?php echo url('password_reset').'?key='.$_GET['key'].'&confirm_reset=yes'; ?>" class="btn btn-primary btn-block btn-lg">Reset Password</a>
            <br />
